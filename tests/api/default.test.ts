...
describe('API handler', () => {
  it('should handle missing username', async () => {
    const req = { url: 'http://example.com', get: vi.fn().mockReturnValue('example.com'), protocol: 'http' } as any;
    const res = { status: vi.fn().mockReturnThis(), send: vi.fn() } as any;
    await handler(req, res);
    expect(res.status).toHaveBeenCalledWith(400);
    expect(res.send).toHaveBeenCalledWith(expect.stringContaining(`Missing Required Parameter`));
  });
  ...
  it('should handle errors', async () => {
    const req = { url: 'http://example.com?username=testuser' } as any;
    const res = { status: vi.fn().mockReturnThis(), send: vi.fn() } as any;
    (getBubbleData as Mock).mockRejectedValue(new Error('Generic failed to fetch'));
